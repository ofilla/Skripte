...
Since this must be valid for all $r$, all $p_r$ need to be the same. To match the usual notation, $k=k_B$ and $\Omega=N$ are to be set.
...
Thus, $\lambda_2$ needs to be described by the temperature $T$:
...
\begin{eqnarray}
    ...
    \Rightarrow p_r &=& \frac{1}{N} \exp(- \frac{E_r}{kT} - \frac{\mu N}{k}) \\
...
Still, all coins have the same amount of information. The probability to choose a random coin is the same for all coins.
...
There are $9$ bits of information to identify. Since there are $S$ bits of information in total and $I$ can be gained in one measurement, the number of minimal required measurements $N=2$ can be computed.
...
### d) experiment
> In order to actually identify the defective coin in two weighings, you need to ensure that each weighing yields exactly I bits of information. Construct a protocol to do this.
