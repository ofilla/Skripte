# Organisatorisches
Die Gruppenarbeit wird am Abend des 25. Januar '24 abgeben, am 26.01. ist die Prüfung.

Es gibt Für die 10 Aufgaben werden jeweils ca 5 - 20 Minuten geplant.
Fallstudie: 4-5 Leute pro Gruppe, 1 Dokument gemeinsam erstellen.

"offizieller" Kurs der IHK ist knapper.
Es gib 24 [relevante Normen](http://gpm-hochschulen.de/organisation/normen/).

## Prüfung
* Prüfungsrelevant: Methoden zur Bearbeitung der Fallstudie
* Teilweise Definitionen (werden als solche angemerkt)
* Zusatzfragen werden im Kurs schon gestellt.
* Es werden ca Anfang Dezember mögliche Fragen zum Üben verteilt.

Schriftliche Prüfung (90 Minuten) + Mündliche Prüfung zur Fallstudie (ca 10 Minuten je Person), gesamte Gruppe anwesend. Gesamte Fallstudie Prüfungsrelevant.

# Einsendeaufgaben
## 1. Modul 1
### 1. [Projektdefinition](https://www.ihk-koeln-absolventen.de/mod/assign/view.php?id=22018)
### 2. [Ausgestaltung des Projektmanagements](https://www.ihk-koeln-absolventen.de/mod/assign/view.php?id=22019)
### 3. [Multiprojektmanagement](https://www.ihk-koeln-absolventen.de/mod/assign/view.php?id=22020)


# [[20231113085147|Teambuilding]]
## [[20231113085416|Dreieck der Gemeinsamkeiten]]
## Biography-Sharing


# [[20230916115018|Projektmanagement]]
## Grundlagen
* Umfeld Organisationen: VUCA
    * Frage: Modell?

### magisches Dreieck
Quelle: PDF1, Folie 19
**Ziele der Projektleitung**: Projekterfolg sichern

Die essenziellen Parameter Leistung, Kosten und Zeit bedingen und beeinflussen sich gegenseitig.

#### Projektziel
Ergebnis-orientierte, messbare, strategische Zielsetzung, Soll-Zustand / Big Picture
Zitat: PDF1, folie 23
#### Scope / Umfang
Das magische Dreieck: Welche Leistungen sind inbegriffen, welche Kosten und Zeit sind geplant / gebucht.

#### Objectives / operative Projektziele- Deliverables
zu erarbeitende Ergebnisse, können SMART gemessen werden.

### Methoden
#### Vision zu Projektbeginn
Stell dir vor, das Projekt sei umgesetzt. 
* Wer ist betroffen oder involviert?
* Welche Aufgaben haben sich durch das Projekt geändert?
* Welche Arbeitsabläufe haben sich geändert?
    * Wer bekommt neue Aufgaben?
    * Wer gibt Aufgaben ab?
* Welche Ressourcen werden für das Projekt benötigt?
* Mit welchen Widerständen muss man rechnen?
* Wie soll wer über den Stand der Dinge informiert werden?

Finale Frage: Wer muss was wann tun, damit das Projekt realisiert wird.

## Definitionen
## Projekt
Wir lernen das Projektmanagement nach DIN 69501 (PDF1, Folie 50).

Ein Projekt ist ein Vorhaben, das im Wesentlichen durch die *Einmaligkeit der Bedingungen in Gesamtheit*, eine *genaue Zielvorgabe*, *zeitliche, finanzielle und personelle Begrenzungen* gekennzeichnet, das *projektspezifisch organisiert* ist.

*Projektspezifische Organisation* bedeutet, dass mehrere Leute zusammenarbeiten.

* neu, innovativ?
* komplex: Es sind verschiedene Personen / Abteilungen / ... betroffen / involviert
* Zeitaufwendig: Mitarbeitende müssen von anderen Aufgaben /... abgestellt werden, es geht nicht nebenbei
* Klarer Zeitrahmen

#### Klassifizierung von Projekten
* nach Art (nach DIN 69501)
    * IT-Projekt (inkl. Schulung von Anwendern)
    * Organisationsprojekt: Schaffung oder Veränderung von internen Strukturen, Change Management
        * z.B. Qualitätsmanagement einführen, neue Normen einhalten
    * Investitionsprojekt: Anschaffung, Geldmittel in Sachvermögen investieren
    * Innovationsprojekt: eine Neuheit entwickeln und umsetzen, bis hin zur Markteinführung
    * Forschungs- und Entwicklungsprojekt: neue Kenntnisse und Fertigkeiten entwickeln

weitere Kriterien
* Produktart: Print, digital, cross-media
* Auftraggeberart: intern  / extern
* räumliche Ausdehnung: lokal, national, international
* Klasse: groß, klein, fein, A, B, C

### Projektmanagement
Ein Projekt durch die verschiedenen Phasen begleiten / leiten.

## Management Regelkreislauf
PDF1, Folie 51

# Projektorganisation
Es gibt im Projektmanagement immer eine Entscheidungsebene (den Auftraggeber), eine Koordinationsebene (die Projektleitung) und eine Ausführungsebene (das gesamte Team).

Die Organisation kann in einem *Organigramm* dargestellt werden.

## Projektorganisationsformen
Es müssen Zuständigkeiten geklärt werden. Es gibt Einflussprojekte, autonome Projekte und Matrixprojekte.

Auswahlidee: PDF2, Folie 36

### Einflussprojekte (Linienprojekt / Stabsprojekt)
Die Entscheidungsebene, die Koordinationsebene und die Ausführungsebene sind vollständig getrennt. Diese Projektform ist oft schwer zu managen, da der Projektmanager keine Weisungsbefugnis hat.

### autonome / reine Projekte
In einem autonomen Projekt sind Entscheidungsebene, die Koordinationsebene und die Ausführungsebene im selben Block, der Projektmanger ist den Teammitgliedern gegenüber weisungsbefugt. Daher ist es einfacher zu managen als Einflussprojekte.

Diese Projekte sind bei hoher Dringlichkeit am effizientesten.

### Matrixprojekte
Diese Form tritt meistens auf. Das Team ist aus verschiedenen Bereichen zusammengestellt. Die Teammitglieder arbeiten nicht nur an dem Projekt, sondern arbeiten auch an anderen Stellen.

#### Regeltermine
Insbesondere bei Matrixprojekten ist es sinnvoll, Termine regelmäßig zu planen.


## Zusammenstellung Projekteam


## Multiprojektmanagement
Das *Portfolio* ist das, was ein Unternehmen anbietet. Es umschließt alle Projekte und Programme.

Ein *Programm* besteht aus einer Reihe von Projekten, die das selbe Programmziel anstreben. Auch ein Programm hat eine definierte Laufzeit, hat aber einen so großen Umfang, dass es aus mehreren Projekten besteht.

Es gibt immer Spannungen zwischen verschiedenen Projekten. Es gibt strategische Ziele, nach denen man wählt, welche Projekte durchgeführt werden, und welche parallel oder nacheinander ausgeführt werden müssen.

Dabei werden die Projekte ausgewählt, die den grössten Nutzen bringen. Es sollten nicht zu viele Projekte mit großem Risiko ausgeführt werden.

Es müssen Ressourcenkonflikte vermieden werden, dazu gehören auch zeitliche Engpässe. Die Bewertung der Projekte sollte nach *Attraktivität* und *Risiko* erfolgen. Die Auswahl wird nach *Strategischer Auswahl* und *Dringlichkeit*.

Zudem sind übergreifende Berichte und Wissensmanagement wichtig.

Eine Pipeline oder eine Roadmap kann Überblick über verschiedene Projekte schaffen (PDF2, Folie23).
Verschiedene Projekte und ihre Beziehungen können in Strategischen Projektnetzen (PDF2, Folie 24) dargestellt werden.

Ein Portfolio-Trichter (PDF2, Folie 25) kann den Zustand des Portfolios und den aktuellen Status von Projekten und Programmen darstellen.

## Projektmanagement Office (PMO)
Das PMO ist dazu da, die eigentlichen Projektmanager zu unterstützen. Es ist insbesondere dazu da, Standards vorzulegen und Vorlagen zu liefern. Dazu werden Management-Prozesse standardisiert und optimiert.

Weiterhin soll das PMO alle laufenden Projekte im Überblick behalten.

## Vorgehensmodelle
### plangetriebenes Projektmanagement
Wenn man weiß, was wie erreicht werden kann. Ein solches Projekt ist in die [[20231026135352|Phasen des Projektmanagements]] aufzuteilen.

#### Def: Meilenstein
Meilensteine kennzeichnen Start- oder Endpunkte einer Phase. Diese können eine [[20231026135352|Phase des Projektmanagements]] sein, aber insbesondere die Steuerungsphase kann in Unterphasen aufgeteilt werden, die wiederum durch Meilensteine getrennt werden. Dies ist beispielsweise bei SCRUM der Fall. [Methodenwürfel](https://www.gpm-hochschulen.de/methoden/methodenwuerfel/scrum/).

1. Die *Projektidee* bildet den Anfang der [[20231026140539|Initialisierungsphase]].
2. Die Genehmigung des *Projektantrags* bildet den Anfang der [[20231027144509|Planungsphase]].
3. Die Erteilung des *Projektauftrags* bildet den Anfang der [[20231027152443|Steuerungsphase]].
4. Die [[20231027155033|Abnahme]] bildet das Ende der [[20231027152443|Steuerungsphase]].
5. Die Bestätigung des Abschlusses bildet das Ende der [[20231027155339|Abschlussphase]].

An Meilensteinen wird oft die Frage gestellt, ob das Projekt fortgeführt werden soll.

### agiles Projektmanagement
Das agile Projektmanagement ist die Antwort auf die zunehmende Komplexität und den Wandel von Projekten, gerade in der IT. Es bedeutet mehr Selbstorganisation. Besonders bei sehr komplexen Problemen kann es sinnvoller sein.

Anders als im plangetriebenen Projektmanagement ist diese Form sehr dynamisch und flexibel. Es soll dabei geringere Anteile an Planungs- und Führungsintensität geben.

Agil ist eine Einstellung, zu dem es keine eindeutige Vorlage gibt. Wenn man nicht genau weiß, was rauskommen soll, oder Methoden nicht bekannt sind. Die Arbeit erfolgt in Sprints.

Eine agile Methode ist SCRUM.

#### Methode
PDF1, Folie 55

Am Ende eines jeden Sprints gibt es ein Zwischenergebnis.

### Hybrid
Das hybride Projektmanagement ist eine Mischung aus dem plangetriebenen Projektmanagement und dem agilen Projektmanagement.

Insbesondere in der [[20231027152443|Steuerungsphase]] kommen agile Methoden zum Einsatz. Wenn nötig, wird in die [[20231027144509|Planungsphase]] zurückgesprungen.

### Stacey-Analyse
Die Stacey-Analyse kann helfen, ein passendes Vorgehensmodell zum Projektmanagement zu finden.

PDF1, Folie 61

### Kanban
Bei der Kanban-Methode werden Aufgaben auf einzelne Karten verteilt, die jeweils verschiedene Phasen durchlaufen. Es ist eine Visualisierungstechnik, die eine schnelle Durchlaufzeit von Aufgaben ermöglicht.

Üblich sind die Phasen "To Do", "Doing" und "Done", auch "Testing" oder "Review" sind möglich.

#### Kanban-Regeln
* Jedes Arbeitspaket hat eine Karte
* In Teilprozesse gliedern
* Immer sofort notieren, nicht im Kopf herumtragen
* Nicht zu kleinteilig werden / bei wenigen Spalten bleiben
* Tagesplan / Wochenplan erstellen
* Fristen hinterlegen bei notwendigen Deadlines

## Projektmanagement einführen
PDF1, Folien 72ff

PDF2, Folie 35

## Initialisierungsphase
Zu Beginn klarheit schaffen (PDF1, Folie 81)
1. Handelt es sich um ein Projekt (Anforderungen)?
2. Welche Informationen sind bekannt?
3. Was ist klar, unklar, strittig, wer sind Schlüsselpersonen?

1. Ideensammlung: -> Projektideen-Formular
2. Bewertungskriterien
3. Projektsteckbrief (PDF1, Folie 85) -> Projektvorhaben / Projektantrag
4. Freigabe (durch Auftraggeber)

Projekte bewerten: PDF1, Folie 86

## Definitionsphase
* Erste Kernpunkte für den Projektauftrag klären
* Konkrete Ziele definieren
* Umfeld & Stakeholder analysieren
* Kernteam bilden
* Grobstruktur und Rahmenbedingungen (Lastenheft/Pflichtenheft)
* Unterschriebener formaler Projektauftrag

Der konkrete Auftrag muss definiert werden. Dabei muss festgelegt werden, welche Ziele erreicht werden sollen. Davon müssen andere Ziele abgegrenzt werden, die nicht in den Projektinhalt gehören.

Dann kann ein möglicher Projektauftrag erstellt werden, der die Kernpunkte enthält.

Dabei hilft oft eine Ziel- und Auftragsklärung, beispielsweise in einem Analyse-Gespräch, einer Fragerunde oder einem Fragekatalog. Am Ende muss ein schriftlicher Projektauftrag erstellt werden.

## Projektauftrag
PDF2, Folie 44
Üblicherweise werden in einem Projektauftrag folgende Fragen beantwortet:

1. Warum?
2. Was?
3. Wer (ist involviert)?
4. Wie?
5. Wieviel (Kosten)?
6. Wann?
7. Wie gut?

Bei externen Dienstleistungen handelt es sich um einen Projektvertrag.

### Lastenheft
Was muss geleistet werden?

### Pflichtenheft
Was muss der Auftraggeber machen?

# Methoden
| Initializierungsphase            | Definitionsphase             | Planungsphase | Steuerungsphase | Abschlussphase |
| -------------------------------- | ---------------------------- | ------------- | --------------- | -------------- |
| Projektideen prüfen & beurteilen | Projektauftragsklärung       |               |                 |                |
| Situationsanalyse                | Ziele definieren             |               |                 |                |
|                                  | Meilensteine & Kosten (grob) |               |                 |                |

## Initialisierung
### IST-Analyse
DIN: I.11.1

* Welchen Auslöser gibt es für das Projekt? 
* Wodurch ist die Ausgangssituation gekennzeichnet?
* Welche Schwachstellen und Probleme bestehen derzeit? Mögliche Ursachen?
* Wo besteht Optimierungsbedarf?
* Wie ist die Projekthistorie?


### SWOT-Analyse
Visualisierung: PDF2, Folie 11

|               |            | 
| ------------- | ---------- |
| Strengths     | Weaknesses |
| Opportunities | Threats    |

### Projekt prüfen und einordnen
Ist es wirklich ein Projekt (und keine Aufgabe)? Welche Projektart liegt vor?
-> Projektsteckbrief

## Definition
### Anforderungen klären
D.6.1 - PDF2, Folie 47

#### Priorisierung: MoSCoW-Prinzip
Anforderungen werden nach Must, Should, Could und Won´t klassifiziert. Damit können sie priorisiert werden.

## Ziele definieren
Es werden Zustände definiert ("Am Ende liegt [...] vor"). Was liegt am Ende der Steuerungsphase vor? Was wird abgenommen? Was wird freigegeben / abgenommen? Positive Formulierungen (außer Nicht-Zielen).

#### Zielhierarchien & -klassen
##### Induktiv: Bottom-Up-Methode
##### Deduktiv: Top-Down-Methode

### S.M.A.R.T


# Literatur
* [Methodenwürfel](http://gpm-hochschulen.de/methoden/methodenwuerfel/) zu Methoden der verschiedenen Phasen
* Günter Drews / Norbert Hillebrand: Lexikon der Projektmanagement-Methoden. München: Rudolf Haufe Verlag GmbH & Co.KG
